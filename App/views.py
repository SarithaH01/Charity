...
from .models import userProfile,Event,Donation,Account,Blog,Track,Feedback
...
def homepage(request):
    ...
    return render(request,'index.html',context = {'profile':profile,"blogs":blog[:2],'feeds':Feedback.objects.all()})    # the index.html template can use it to render the page dynamically
...
def addFeed(request):
    ...
    Feedback.objects.create(user = request.user,comment = request.POST.get('comment'),sentiment = predicted_sentiment)
