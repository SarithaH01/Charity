...
def donate(request):
    ...
    contact_list = w3.eth.contract(address=transaction_receipt.contractAddress, abi=abi)
    store_contact = contact_list.functions.addDonationDetails(
        request.user.username,post.user.username,str(formatted_time),eventHost.account_address,request.POST.get('amount')
    ).buildTransaction({"chainId": chain_id, "from": address, "gasPrice": w3.eth.gas_price, "nonce": nonce + 1})
